package qr.qrcodescanner.barcodescanner.barcodereader.qrcode.qrreader.scan.feature.common.updater
...
class InAppUpdater(val activity: BottomTabsActivity) {
    ...
    companion object {
        ...
        fun getInstance(bottomTabsActivity: BottomTabsActivity): InAppUpdater {
            return INSTANCE ?: InAppUpdater(bottomTabsActivity).apply { INSTANCE = this }
        }
    }

    fun checkInAppUpdates(activity: BottomTabsActivity) { // For showing immediate again
        ...
        appUpdateService?.flexibleUpdateWithDays?.observe(activity, Observer {
            ...
            if (it) {
                ...
                appUpdateService?.startFlexibleUpdateWithDays(
                    activity,
                    BottomTabsActivity.IN_APP_UPDATE_REQUEST_CODE
                )
            }
        })
    ...
    private fun popupSnackBarForCompleteUpdate() { // Test
        ...
        Snackbar.make(
            ...
            setActionTextColor(activity.resources.getColor(R.color.new_secondary_color))
            show()
        }
    }
