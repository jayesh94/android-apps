...
import qr.qrcodescanner.barcodescanner.barcodereader.qrcode.qrreader.scan.feature.common.dialog.ConfirmBarcodeDialogFragment
...
class ScanBarcodeFromCameraFragment(private val barcodeScan: Boolean = false) : Fragment(), ConfirmBarcodeDialogFragment.Listener {
...
//        requestPermissions()
    ...
    override fun onResume() {
        ...
        faLogEvents.logScreenViewEvent("ScanBarcodeFromCameraFragment", "ScanBarcodeFromCameraFragment")
...
//        if (areAllPermissionsGranted()) {
    ...
    private fun initScanner(barcodeScan: Boolean = false) {

        if (barcodeScan){
            scanner_view.frameAspectRatioHeight = 2F
            scanner_view.frameAspectRatioWidth = 3.5F
            scanner_view.frameSize = 0.9F
        }
    ...
    private fun initFlashButton() {
        layout_flash_container.setOnClickListener {
            faLogEvents.logCustomButtonClickEvent("fragment_scan_barcode_from_camera_toggle_flash")
            toggleFlash()
        }
    ...
    private fun handleScanFromFileClicked() {
        layout_scan_from_file_container.setOnClickListener {
            faLogEvents.logCustomButtonClickEvent("fragment_scan_barcode_from_camera_scan_from_file")
    ...
    private fun handleZoomChanged() {
        seek_bar_zoom.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onStartTrackingTouch(seekBar: SeekBar?) {
                faLogEvents.logCustomButtonClickEvent("fragment_scan_barcode_from_camera_seek_bar_zoom")
            }
